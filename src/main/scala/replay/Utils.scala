...
object genVerilogFragment {
  // Generate verilog harness fragment
  def apply(dutName: String, io: Seq[Data], writer: java.io.Writer) {
    val (inputs, outputs) = (getDataNames(io)
      filterNot { case (port, name) => name == "clock" || name == "reset" }
      partition { case (port, name) => directionOf(port) == ActualDirection.Input })
    ...
    writer write (inputs.unzip._2 map (name => s"    .${name}(${name})") mkString ",\n")
    if (inputs.nonEmpty) writer write ",\n"
    writer write (outputs.unzip._2 map (name => s"    .${name}(${name}_delay)") mkString ",\n")
    writer write "\n  );\n\n"
